        ...
        async function LoadBlockchaindata(){
          ...
          if(networkData){
            ...
            for(var i=1;i<=CandidateCount;i++)
            {
                ...
                win=cand;
                Setcandidates((candidates)=>[...Array.from(new Set(candidates)),createData(i,cand.id,cand.name,cand.age,cand.party,cand.qualification,parseInt(cand.voteCount))]);
  ...
  marginTop:  '18px',marginLeft: "5px"}}>{voteraddress}</span>
    ...
    {currState==="Voting"?
      <div style={{display: "flex"}}>
       ...
       {
        candidates.map((row)=>(
          <VoterCard img={img4} idx={row.candidate_id} name={row.name} age={row.age} party={row.party} qualification={row.qualification} button={false}/>
        ))
       }
       </div>:
